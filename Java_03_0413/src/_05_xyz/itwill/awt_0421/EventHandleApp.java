...
// 컴퍼넌트에서는 다양한 이벤트가 발생되며 발생될 경우 원하는 기능을 구현하여 실행되도록 프로그램 작성
// └-> 이벤트 프로그램(이벤트 처리)
...
// => 컴퍼넌트에서 발생된 이벤트를 처리하기 위해 Listener 인터페이스에서 추상메소드 제공
// => Listener 인터페이스를 상속받아 자식클래스에서 추상메소드를 오버라이드 선언하여  
//    오버라이드 선언된 메소드에 이벤트 처리 명령 작성

// 3. 이벤트 소스에서  이벤트가 발생되면 이벤트 처리 클래스의 객체를 제공받아 실행되도록 설정
// => 형식) Component.addXXXListener(Listener l)  : 컴퍼넌트의 이벤트를 처리하기 위한, 이벤트 처리 객체 등록하는 메소드
// => 이벤트 처리 객체를 제공받아 자동으로 이벤트 처리 메소드를 호출하여 이벤트 처리



// [Exit] 버튼을 누른경우 프로그램 종료하는 기능의 프로그램 작성
public class EventHandleApp extends Frame {
	...
	public EventHandleApp(String title) {
		...
		// 이벤트 처리 생성 (컴퍼넌트에 이벤트 처리 객체 등록)
		// => Button 컴퍼넌트에서 ActionEvent가 발생될 경우EventHandle 클래스의 객체를 제공받아
		//    이벤트 처리 메소드를 자동 호출하여 이벤트 처리
