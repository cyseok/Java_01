...
<%
	...
	// => PostMethod 객체 -> post 방식
	GetMethod method = new GetMethod(url);
	
	try {
		// httpclient.HttpClient.executeMethod(HttpMethod method) : 가상의 브라우저를 사용하여 웹프로그램을 요청
		// => 웹프로그램 요청에 대한 상태 코드 반환
		int statusCode = client.executeMethod(method);
		
		// 클라이언트에게 응답 처리 하기 위한 프록시 프로그램의 response 객체 초기화
		response.reset();
		
		// 프록시 프로그램의 출력 스트림 초기화
		out.clearBuffer();
		
		// 프록시 프로그램을 요청한 클라이언트에게 상태코드 전달
		response.setStatus(statusCode);
		
		if(statusCode==HttpServletResponse.SC_OK) {  // -> 정상적인 실행결과
			// 몸체부의 결과값을 문자열로 반환 -> 결과값을 반환받아 문자형태로 캐릭터셋 설정
			String result = new String(method.getResponseBodyAsString().getBytes("8859_1"),"utf-8");
			
			// 프록시 프로그램을 요청한 클라이언트에게 실행결과에 대한 문서형태를 전달
