...
// └-> 클라이언트는 응답받은 쿠키를 저장
// Cookie : 서버(웹프로그램)와 클라이언트(브라우저)의 연결 지속성을 제공하기 위해 클라이언트에 저장되는 문잔ㄴ값
...
public class CookieCreateServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Cookie 객체 생성 -> Cookie 객체를 클라이언트에 전달하여 클리이언트에 쿠키 저장
		...
		// 클라이언트에 전달되 저장될 쿠키의 유지시간 설정
		// Cookie.setMaxAge(int expiry) : 쿠키의 유지시간을 변경하는 메소드
		// => 매개변수에 쿠키를 유지하기 위한 시간(초)을 전달
		// setMaxAge() 메소드를 호출하지 않은 경우 쿠키 유지시간은 (-1)로 설정됨
		// => 유지시간이 (-1)로 설정된 쿠키는 브라우저 종료시 자동 소멸됨
		countCookie.setMaxAge(24*60*60);  // 쿠키의 유지시간을 1일로 변경
		
		// 클라이언트에게 Cookie 객체 전달 -> 클라이언트에  쿠키 저장
		// => Cookie 객체의 유지 시간이 (-1)로 설정된 쿠키는 클라이언트 브라우저 메모리에 저장 -> 브라우저 종료시 소멸
		// => Cookie 객체의 유지 시간이 (-1)이 아닌 쿠키는 클라이언트 쿠키 파일로 저장 -> 유지시간 경과 후 소멸
