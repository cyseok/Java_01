...
// @WebServlet 어노테이션 속성을 이용하여 서블릿의 이름과 URL 패턴을 매핑 설정
// => name 속성 : 서블릿의 이름을 속성값으로 설정 -> 속성을 생략하면 클래스 이름이 서블릿의 이름으로 설정됨
// => ex) @WebServlet(name="name", value="/First.itwill")
// => value 속성 : 서블릿을 요청하기 위한 URL패턴을 속성값으로 설정 
// └-> value 속성외 다른속성이 없는 경우 속성값만 설정 가능
