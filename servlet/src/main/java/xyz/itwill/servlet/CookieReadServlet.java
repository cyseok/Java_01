...
// => 클라이언트가 서버에 접속하여 웹프로그램을 요청할 때 서버 정보로 저장된 모든 쿠키를
//     리퀘스트 메시지에 담아 제공
...
public class CookieReadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 클라이언트로 전달받은 모든 쿠키를 반환받아 저장
		// HttpServletRequest.getCookie() : 클라이언트에게 전달받은 모든 쿠키를 얻어와
		...
		out.println("<hr>");
		if(cookies == null) {
			out.println("<p>쿠키없음</p>");
		} else {
			String id="";
			String count="";
			
			// Cookie 객체 배열의 요소를 차례대로 제공받아 사용하기 위한 반복문
			for(Cookie cookie : cookies) {
				// 클라이언트로부터 전달받은 쿠키에서 필요로하는 쿠키값을 반환받아 저장
			...
			// 클라이언트에게 쿠키 값을 HTML 문서에 포함하여 출력
