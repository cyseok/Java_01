...
public class FileDownload extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		//
		// 클라이언트에게 파일을 전달하기 위해 리스폰즈 메세지의 머리부 변경
		response.setContentType(getContentType());
		response.setContentLengthLong((int)downloadFile.length());
		//=============================================================================
		// ★다운로드 처리되어 클라이언트에 저장될 파일명을 리스폰즈 메세지의 머리부에 저장하여 전달
		...
		//
		//FileCopyUtils.copy(InputStream in, OutputStream out) : 입력스트림으로 원시데이타를 반복적으로 읽어 출력스트림으로 전달하는 메소드 - 파일 복사 
