...
// 4. 테이블 생성 -> DTO 클래스 작성 -> mapper 파일 작성 -> DAO 클래스 작성

// DAO 클래스 : 저장매체(DBMS)에게 행에 대한 삽입, 변경, 삭제, 검색 기능을 제공하기 위한 클래스
// => DAO 클래스의 메소드에서는 DBMS 서버에 SQL 명령을 전달하여 실행하고 실행결과를 Java 객체(값)로 반환 되도록 작성
...
@Repository
// final 제한자로 필드만 초기화 설정하는 생성자를 만들어 주는 어노테이션
