...
public class MethodController {
	...
	public String inputOne() {  // 일반 메소드지만 @RequestMapping을 사용하여 요청 처리 메소드로 바뀜
		return "method_input1";  // method_input1을 반환
	}
	
	// 입력값을 반환받아 Request Scope 속성값으로 저장하고 속성값을 출력하기 위한 JSP 문서의 ViesName을 반환하는 요청 처리 메소드
	...
	public String outputOne(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(request.getMethod().equals("GET")) {
			...
			response.sendRedirect("method_input");//클라이언트에게 URL 주소 전달 - 재요청
			return null;
		}
		request.setCharacterEncoding("utf-8");//UnsupportedEncodingException 에러 발생 → 떠넘기기
	...
	// 요청 처리 메소드를 호출하는 요청 URL 주소가 같아도 요청방식을 다르게 지정하면 다른 요청 처리 메소드로 매핑 처리
