...
//
//모델(Model) 역할을 제공하는 클래스 - 요청 처리 클래스
...
public class HelloController {
	...
	// => Front Controller는 제공받느 뷰이름을 이용하여 응답 관련 정보로 변환(ViewResolver)하여 응답 처리
	//
	// Front Controller에 의해 JSP 문서로 포워드 이동하여 응답되도록 뷰이름을 제공하는 방법
	// 방법1. 요청 처리 메소드의 반환형을 void로 설정한 경우 Front Controller에게 메소드의 이름을 뷰이름으로 제공 
	// 방법2. 요청 처리 메소드의 반환형을 String로 설정한 경우 Front Controller에게 반환값을 뷰이름으로 제공
	// 방법3. 요청 처리 메소드의 반환형을 ModelAndView로 설정한 경우 Front Controller에게 ModelAndView 객체의 뷰이름을 저장하여 제공
	// JSP 문서로 응답 처리하지 않을 경우 요청 처리 메소드의 반환형을 Model 또는 Map으로 설정 
	//
	...
	// => 클라이언트의 요청방식을 구분하여 요청 처리 메소드가 호출되고자 할 경우 @GetMapping, @PostMapping, @PutMapping, @PatchMapping, @DeleteMapping 등의 어노테이션 사용
	// value 속성 : 클라이언트의 요청 URL 주소를 속성값으로 설정
	// => Front Controller에 의해 등록된 클라이언트 요청 URL 주소(식별자)로 요청 처리 메소드가 호출되어 클라이언트의 요청 처리
	// => 다른 속성이 없는 경우 속성값만 설정 가능
	// => ★다른 요청 처리 메소드의 value 속성값과 요청 URL 주소가 중복될 경우 WAS 실행시 에러 발생
	@RequestMapping(value = "/hello")//이게 있어야 밑에있는 hello()가 요청 처리 메소드가 됨
	public void hello() {//요청 처리 메소드
		// 메소드 안에 요청 처리 명령 작성 - Service 객체의 메소드 호출
		logger.info("[/hello] 페이지 요청 : HelloController 클래스의 hello() 메소드 호출");//info 레벨을 출력하려면 log4j.xml logger 엘리먼트 추가해야함
	}
	//
