...
public class ParamController {
	...
	//
	/*
	//요청 처리 메소드에 HttpServletRequest 인터페이스로 매개변수를 선언하면 Front Controller에게 클라이언트의 요청정보가 저장된 HttpServletRequest 객체를 제공받아 사용 가능
	...
	*/
	...
	/*
	// 좀더 쉬운 방법
	// 요청 처리 메소드에서 페이지 요청시 전달되는 값의 이름과 같은 이름으로 String 자료형의 매개변수를 선언하면 
	//	Front Controller에 의해 전달값이 매개변수에 자동으로 저장되어 제공
	// => 전달값의 이름과 매개변수의 이름이 같지 않을 경우 매개변수에는 [null] 저장
	// 클라이언트 요청에 의해 웹프로그램이 실행되기 전과 후에 원하는 명령을 실행할 수 있는 필터를 [web.xml] 파일에 등록하여 사용 가능
	// => 클라이언트의 모든 요청에 대한 요청정보가 저장된 리퀘스트 메시지 몸체부의 문자형식을 변경하는 필터를 사용되도록 [web.xml] 파일을 이용하여 설정 - 인코딩 필터
	// => 필터는 Front Controller 앞에 위치되어 요청과 응답 관련 정보 변경 가능 - WAS에 의해 제어
	@RequestMapping(value = "/param", method = RequestMethod.POST)
	...
	*/
	...
	/*
	//전달값을 저장하기 위한 매개변수 @RequestParam 어노테이션 사용
	...
	*/
	...
	// => 전달값의 이름과 매개변수의 이름이 다른경우 사용
	// => 다른 속성이 없는 경우 속성값만 설정 가능
	 
	// required 속성 : false(선택) 또는 true(필수 - 기본값) 중 하나를 속성값으로 설정
	// => 전달값이 매개변수에 전달되어 저장되는 필수 여부를 구분하기 위한 속성
	@RequestMapping(value = "/param", method = RequestMethod.POST)
	...
	*/
	...
	// => 전달값이 없는 경우에도 매개변수 null 대신 저장될 기본값으로 설정
	@RequestMapping(value = "/param", method = RequestMethod.POST)
