...
public class JoinController {
	...
	public String join() {
		return "join_form";   /// join_form 응답 요청
	}
	/*
	//전달값을 받아다가 매개변수에 저장
	//전달값을 제공받아 Request Scope 속성값으로 저장하고 속성값을 출력하기 위한 JSP 문서의 뷰이름을 반환하는 요청 처리 메소드
	...
	*/
	...
	/* VO 클래스 생성해서 메소드 작성
	//전달값을 매개변수로 제공받아 객체의 필드값으로 저장하고 Request Scope 속성값으로 저장하고 속성값을 출력하기 위한 JSP 문서의 뷰이름을 반환하는 요청 처리 메소드
	@RequestMapping(value = "join", method = RequestMethod.POST)
	public String join(@RequestParam String id, @RequestParam String passwd
			, @RequestParam String name, @RequestParam String email, Model model) {
	...
	*/
	...
	public String join(@RequestParam String id, @RequestParam String passwd
		...
		//
		// model.addAttribute("member", member);
		// Modle.addAttribute(Object attributeValue) : Model 객체에 속성명과 속성값(객체)을 저장하기 위한 메소드 - Request Scope
		// => 속성값의 자료형(클래스)을 속성명으로 자동 설정 - 첫문자는 소문자
	...
	*/
	...
	/*
	//★요청 페이지의 전달값이 있는 경우 요청 처리 메소드의 매개변수 자료형을 VO 클래스(DTO 클래스)로 작성하면 
	//	Front Controller는 VO 클래스를 객체로 생성하여 매개변수에 전달하여 저장
	// => VO 클래스의 객체 필드에는 전달값의 이름과 같은 필드에 전달값을 제공받아 저장
	// Command 객체 : 전달값을 제공받아 필드에 저장된 객체로 요청 처리 메소드의 뷰에게 속성값으로 저장하여 사용될 수 있도록 제공
	...
	*/
	...
	public String join(@ModelAttribute(value = "mem") Member member, Model model) {
		if(member.getId().equals("asd123")) {//아이디가 중복된 경우
	...
	*/
	// ★요청 페이지의 전달값이 있는 경우 요청 처리 메소드의 매개변수 자료형으 Map 인터페이스로 작성하면 Front Controller는 Map 객체를 생성하여 매개변수에 전달하여 저장
	// => Map 객체는 전달값의 이름을 맵키(MapKey - String)로 설정하여 전달값을 맵값(MapValue)로 제공받아 엔트리로 추가
	// => Map 인터페이스로 작성된 매개변수에는 반드시 @RequestParam 어노테이션을 사용해야만 Map 객체의 맵값으로 전달값을 제공받아 사용 가능
	@RequestMapping(value = "join", method = RequestMethod.POST)
	public String join(@RequestParam Map<String, Object> map, Model model) {
		model.addAttribute("mem", model);
		return "join_display";
	}
