...
//
//컨트롤러(Controller) : 클라이언트의 모든 요청을 받아 URL 주솔르 분석하여 요청 처리 클래스(Model)의 메소드를 호출하여
//	데이터 처리 후 JSP 문서(View)로 스레드를 이동하여 응답되도록 프로그램의 흐름을 제어하는 기능을 제공
//
//컨트롤러 역활을 제공하기 위한 클래스 - 서블릿 클래스
// => 클라이언트의 모든 요청을 받아 처리하는 단일 진입점의 역할을 수행하는 서블릿 - Front Controller Pattern
// => web.xml 파일에서 클래스를 서블릿(웹프로그램)으로 등록하고 클라이언트의 모든 요청을 처리할 수 있도록 URL 주소의 패턴을 설정
public class DispatcherServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 클라이언트의 요구사항을 이용하여 요청 처리 클래스(Model)의 객체를 제공받아 객체의 메소드를 호출하여 클라이언트 요청에 대한 처리 작업
		/*
		//비효율적인 방법
		...
		*/
		...
		//HandlerMapping 객체의 메소드를 호출하여 클라이언트의 요구사항을 처리하기 위한 요청 처리 클래스의 객체를 반환받아 저장
		Controller controller=handlerMapping.getController(command);
		//
		// 요청 처리 클래스의 메소드 호출하여 요청을 처리하고 JSP 문서의 이름(ViewName)을 반환받아 저장
