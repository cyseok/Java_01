...
public class CreateBeanApp {
	public static void main(String[] args) {
		...
		// BeanFactoryg 인터페이스를 상속받은 자식클래스로 객체 생성 -> BeanFactory 객체
		...
		//		생성하지 않고 Spring Bean 객체를 제공받기 위해 요청시 객체를 생성하여 제공
		BeanFactory factory=new XmlBeanFactory(new FileSystemResource("src/main/resources/04-1_beanCreate.xml"));
		System.out.println("=============== Spring Container 초기화 후 ===============");
		
		// BeanFactory.getBean(String beanName) : 매개변수에 Spring Bean을 구분하기 위한 식별자(beanName)를
		//  전달하여 스프링 컨테이너로 부터 Spring Bean 객체를 생성하여 반환하는 메소드
