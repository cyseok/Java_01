...
//          hash() 메소드를 자동 생성하여 제공하는 어노테이션
// @NoArgsConstructor : 매개변수가 없는 생성자를 자동 생성하여 제공하는 어노테이션
// @AllArgsConstructor : 모든 필드에 초기값을 저장할 수 있는 매개변수가 있는 생성자를 자동 생성하여 제공하는 어노테이션
// @RequiredArgsConstructor : 특정필드에 초기값을 저장할 수 있는 매개변수가 있는 생성자를 자동 생성하여 제공하는 어노테이션
// => final 제한자를 사용하여 선언된 필드에 초기값 저장
// => @NoArgsConstructor 과 중복되면 error 
// @Setter : 클래스에 선언된 모든 필드에 대한 Setter 메소드를 자동으로 생성해서 제공해주는 어노테이션
// => 필드에 @Setter 어노테이션을 사용하면 필드에 대한 Setter 메소드를 자동 생성하여 제공
// @Setter : 클래스에 선언된 모든 필드에 대한 Getter 메소드를 자동으로 생성해서 제공해주는 어노테이션
//=> 필드에 @Getter 어노테이션을 사용하면 해당 필드에 대한 Getter 메소드만을 자동 생성하여 제공
// @ToString : 클래스에 toString() 메소드를 자동으로 생성해서 제공해주는 어노테이션
// => 클래스에 선언된 모든 필드값을 문자열로 반환하는 기능 제공
// @Builder : 클래스에 Builder 클래스를 자동 생성하여 제공하는 어노테이션
// => Builder : 객체 생성시 객체 필드에 필요한 값을 저장하기 위한 기능을 제공
// => 생성자보다 가독성이 좋으며 필드의 순서에 상관없이 초기화 작업이 편리
// => 매개변수에 생성자에  @Builder 어노테이션을 사용하여 생성자 매개변수로 초기화 처리하기 위한
//    필드에만 값을 저장할 수 있는 메소드 제공
// @Slf4j : 로그 이벤트를 발생하는 Logger 객체가 저장된 필드를 자동 생성하여 제공하는 어노테이션
// 
