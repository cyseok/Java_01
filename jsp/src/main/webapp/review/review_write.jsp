<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 사용자로부터 게시글을 입력받은 JSP 문서
=> 로그인 상태의 사용자에게만 요청
=> [글저장] 태그를 클릭한 경우 [review/review_write_action.jsp] 문서 요청 -> 게시글정보 전달 --%>

<%-- 새글 : [review_list.jsp] 문서에 의해 [review_write.jsp] 문서를 요청한 경우 -> 전달값 없음 --%>
<%-- 답글 : [review_detail.jsp] 문서에 의해 [review_write.jsp] 문서를 요청한 경우 -> 전달값(처음달았던 답글) 있음
=> [review_detail.jsp] 문서에서 부모 게시글 관련 정보(ref, restep, relevel, pageNum) 전달 --%>

<%-- 비로그인 상태의 사용자가 JSP 문서를 요청한 경우 error_page 이동 --%>
...
<%
	...
	if(request.getParameter("ref")!=null) {  // 전달값이 있는 경우 -> 답글
		...
		pageNum = request.getParameter("restep");
	}
%>
