<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 선언문(Declaration) : 필드 or 메소드를 선언하는 명령을 작성하는 영역 제공
 => 서블릿 클래스 내부에 명령으로 해석  --%>
<%! // 서블릿 클래스의 객체에 필요한 값을 저장하는 필드
	// => 필드는 서블릿 객체 소멸시 자동 소멸됨
	int count=0;
%>
    
<%-- 스크립틀릿(Scriptlet) : 요청을 처리하기 위한 Java 명령을 작성하는 영역 설정
=> 요청 처리 메소드의 명렁으로 해석 --%>
<%  // 요청 처리 메소드에서 생성되어 사용되는 지역변수
	// => 지역변수는 변수가 선언된 메소드 종료시 자동 소멸됨
	// int count = 0;
